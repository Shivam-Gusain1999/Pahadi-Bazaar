    ...
    product && (
      <div onClick={(e) => {
        ...
        className="group relative cursor-pointer p-4 rounded-2xl flex flex-col justify-between
                   shadow-md hover:shadow-2xl hover:-translate-y-2 transition-all duration-300 overflow-hidden bg-white dark:bg-gray-800 dark:shadow-none dark:hover:shadow-gray-700/50"
      >
        {/* Decorative Glow */}
        <div className="absolute inset-0 bg-primary/5 dark:bg-primary/10 opacity-0 group-hover:opacity-100 transition duration-500 rounded-2xl"></div>
        ...
        <button
          ...
          className="absolute top-3 right-3 z-30 p-2 rounded-full bg-white/80 dark:bg-gray-700/80 hover:bg-white dark:hover:bg-gray-600 shadow-sm transition"
        >
          <svg
            className={`w-5 h-5 transition ${isInWishlist(product._id) ? "text-red-500 fill-red-500" : "text-gray-400 dark:text-gray-300 hover:text-red-500"}`}
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
        ...
        <div className="flex items-center justify-center relative z-10 bg-white dark:bg-gray-700 rounded-xl p-2 mb-2">
          <img
            className="group-hover:scale-110 transition-transform duration-300 h-40 object-contain drop-shadow-lg"
            src={product.image[0]}
            alt={product.name}
          />
        </div>
        ...
        <div className="mt-3 relative z-10">
          ...
          <div className="flex items-center gap-1 mt-1">
            ...
            {/* Note: Ideally rating should come from product data */}
            <p className="text-gray-500 text-xs">({Math.floor(Math.random() * 50) + 5})</p>
          </div>
